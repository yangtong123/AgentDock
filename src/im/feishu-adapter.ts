...
interface FeishuEvent {
  header?: { event_type?: string } | undefined;
  /** Legacy card-callback envelope (消息卡片请求网址): chat id and action at top level. */
  open_chat_id?: string | undefined;
  action?: { value?: Record<string, string> } | undefined;
  event?: {
    sender?: { sender_id?: { open_id?: string } } | undefined;
    /** Event-subscription card callbacks carry the chat id here (no message object). */
    context?: { open_chat_id?: string } | undefined;
    message?: { chat_id?: string; message_type?: string; content?: string } | undefined;
    action?: { value?: Record<string, string> } | undefined;
  } | undefined;
}

/**
 * Builds the im/v1/messages request body for a reply. Action-bearing replies
 * become an interactive card: approval gates must be approvable from Feishu,
 * not just from Telegram. Button values carry the same compact payloads the
 * inbound card.action.trigger handler decodes. Shared by the webhook and
 * long-connection transports.
 */
export function feishuMessageBody(reply: ImReply): { receive_id: string; msg_type: string; content: string } {
  ...
  if (buttons.length === 0) return { receive_id: reply.conversationId, msg_type: "text", content: JSON.stringify({ text: reply.text }) };
  return {
...
export class FeishuAdapter implements ImAdapter {
  ...
  async send(reply: ImReply): Promise<void> {
    ...
    await this.fetchImpl("https://open.feishu.cn/open-apis/im/v1/messages?receive_id_type=chat_id", {
      ...
      body: JSON.stringify(feishuMessageBody(reply)),
    });
  }
