...
type ProjectRow = { id: string; name: string; repo_path: string; base_branch: string; worktree_root: string; status: ProjectStatus; max_concurrent_tasks: number; default_workflow_preset: string | null; created_at: string; updated_at: string };
const mapProject = (row: ProjectRow): Project => ({ id: row.id, name: row.name, repoPath: row.repo_path, baseBranch: row.base_branch, worktreeRoot: row.worktree_root, status: row.status, maxConcurrentTasks: row.max_concurrent_tasks, defaultWorkflowPreset: row.default_workflow_preset, createdAt: row.created_at, updatedAt: row.updated_at });
...
export class SqliteProjectRepository implements ProjectRepository {
  ...
  create(project: Project): Project {
    this.db.prepare(`INSERT INTO projects (id,name,repo_path,base_branch,worktree_root,status,max_concurrent_tasks,default_workflow_preset,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)`).run(project.id, project.name, project.repoPath, project.baseBranch, project.worktreeRoot, project.status, project.maxConcurrentTasks, project.defaultWorkflowPreset, project.createdAt, project.updatedAt);
    return project;
  }
